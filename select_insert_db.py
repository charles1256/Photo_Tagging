...
# 需求分析：
# 登录：
# 1.连接mongo，打开manager_user数据库
# 2.打开user_infomation集合（表）
# 3.进行检查是否有该用户(id, user_name. pass)
#    这个用户存在，return true
#      这个用户不存在，return false
# 4.关闭连接（不关吧）

# 注册：
# 1.将新注册的用户名和密码，添加进user_infomation集合（表）
# 2.判断这个用户是否存在
#   这个用户不存在，可以插入了
#   这个用户存在，不可以插入了，
# 3.关闭连接
...
#        self.cursor = myclient.cursor()
        self.table_name = table_name
        self.user_name = user_name
        self.password = password
        app.logger.info("数据库连接成功！")
        mydb = myclient["manager_user"]
        mycol = mydb["user_infomation"]
        mydict = { "user_name": "pengcharles", "pass": "123" }
...
#         self.db=pymysql.connect(host="localhost",user="root",passwd="",db="manager_user",charset="utf8")
